    ...
    public void run() {
        ...
        try {
            ...
            do {
                ...
                if (scaleDirectionPerOpType.get(KinesisOperationType.GET) != null &&
                        scaleDirectionPerOpType.get(KinesisOperationType.GET).equals(scaleDirectionPerOpType.get(KinesisOperationType.PUT))) {
                    //Both Operations point at the same direction -> Go with it!
                    finalScaleDirection = scaleDirectionPerOpType.get(KinesisOperationType.GET);
                } else if (scaleDirectionPerOpType.containsValue(ScaleDirection.UP)) {
                    //At least one Operation points UP -> Scale Up so we're not throttled. (Maybe make this decision configurable at some point...)
                    finalScaleDirection = ScaleDirection.UP;
                } else if (scaleDirectionPerOpType.containsValue(ScaleDirection.DOWN)) {
                    //Only options left are Down on one operation, or nothing on both
                    //If DOWN exists - go with it
